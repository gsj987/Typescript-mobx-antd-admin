      ...
      <ResponsiveContainer minHeight={360}>
        <LineChart data={data}>
          ...
          <Tooltip
            ...
            content={content => {
              const payload = content.payload? content.payload: []
              const list = payload.map((item, key) => <li key={key} className={styles.tipitem}><span className={styles.radiusdot} style={{ background: item.color }} />{`${item.name}:${item.value}`}</li>)
